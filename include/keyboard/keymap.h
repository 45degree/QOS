 ...
 * @since 0.0.1
 * @details
 * 该文件只定义扫描码到字符的简单映射, 并不会判断在按键之前是否已经按下shift, Alt等键.
 * 扫描码映射的结果是一个32位长度的整数, 具体结构如下,
 *
 * | 0 | pad(1位) | alt_r(1位) | alt_l(1位) | ctrl_r(1位) | ctrl_l(1位) | shift_r(1位) | shift_l(1位) | EXT(1位) | ASCII(8位) |
 *
 * 其中低8位代表的是按键对应的ASCII码
 * EXT位表示改案件是否可以显示
 * shift_l位表示在产生该扫描码之间，左边的shift键是否已经按下
 * shift_r位表示在产生该扫描码之间，右边的shift键是否已经按下
 * ctrl_l位表示在产生该扫描码之间，左边的ctrl键是否已经按下
 * ctrl_r位表示在产生该扫描码之间，右边的ctrl键是否已经按下
 * alt_l位表示在产生该扫描码之间，左边的alt键是否已经按下
 * alt_r位表示在产生该扫描码之间，右边的alt键是否已经按下
 * pad位表示在产生该扫描码是否是由小键盘产生的
 */
#ifndef QOS_KEYMAP_H_
#define QOS_KEYMAP_H_
...
#define FLAG_BREAK 0x0080u   /* Break Code */
#define FLAG_EXT 0x0100u     /* Normal function keys	*/
#define FLAG_SHIFT_L 0x0200u /* Shift key */
#define FLAG_SHIFT_R 0x0400u /* Shift key */
#define FLAG_CTRL_L 0x0800u  /* Control key */
#define FLAG_CTRL_R 0x1000u  /* Control key */
#define FLAG_ALT_L 0x2000u   /* Alternate key */
#define FLAG_ALT_R 0x4000u   /* Alternate key */
#define FLAG_PAD 0x8000u     /* keys in num pad */
...
#define MASK_ROW 0x01FF

const u32 keymap[NR_SCAN_CODES][3] = {
